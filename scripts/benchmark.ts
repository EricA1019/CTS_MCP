...
import { ResultCache } from '../src/cache/result_cache.js';
import { ConfigManager } from '../src/config/tool_config.js';
import { checkResponseSize, truncateLargeArrays } from '../src/sampling/index.js';
import fs from 'fs';
import path from 'path';

interface BenchmarkResult {
  name: string;
  category: 'sync' | 'async' | 'cache' | 'validation';
  target: number;
  iterations: number;
  mean: number;
  min: number;
...
function formatResults(suite: BenchmarkSuite): void {
  ...
  for (const result of suite.results) {
    ...
    console.log(
      result.category.toUpperCase().padEnd(20),
      result.name.padEnd(30),
      `<${result.target}ms`.padEnd(10),
      `${result.mean.toFixed(2)}ms`.padEnd(10),
      `${result.p95.toFixed(2)}ms`.padEnd(10),
      status
    );
  }
