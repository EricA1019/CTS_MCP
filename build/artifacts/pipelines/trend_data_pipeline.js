...
export class TrendDataPipeline {
     ...
     */
    async aggregateMetrics(projectPath, weeks = 12) {
        ...
        const weeklyMetrics = await this.aggregateWeeklyMetrics(projectPath, commits, weekBoundaries);
     ...
     */
    async generateSnapshotDataset(projectPath, startDate, endDate, weeks, weekBoundaries) {
     ...
     */
    generateWeekBoundaries(startDate, endDate, weeks) {
        const boundaries = [];
     ...
     * Get Git commits in date range
     */
    getCommitsInRange(projectPath, startDate, endDate) {
        try {
            const sinceDate = new Date(startDate).toISOString();
            const untilDate = new Date(endDate).toISOString();
            const gitLog = execSync(`git log --since="${sinceDate}" --until="${untilDate}" --pretty=format:"%H|%ct" --all`, { cwd: projectPath, encoding: 'utf-8', maxBuffer: 10 * 1024 * 1024 });
            if (!gitLog.trim()) {
                return [];
            }
            return gitLog.split('\n').map(line => {
     ...
     */
    async aggregateWeeklyMetrics(projectPath, commits, weekBoundaries) {
     ...
     */
    async calculateMetricsAtCommit(projectPath, commitHash) {
        try {
     ...
     */
    estimateCoverage(projectPath, loc, tests) {
        if (loc === 0)
            return 0;
     ...
     */
    interpolateMissingData(weeklyMetrics, weekBoundaries) {
        const loc = [];
        const tests = [];
        const coverage = [];
        for (let i = 1; i < weekBoundaries.length; i++) {
     ...
     */
    interpolatePoint(weeklyMetrics, weekBoundaries, index) {
...
//# sourceMappingURL=trend_data_pipeline.js.map