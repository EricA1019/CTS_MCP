...
export function createAnalyzeProjectHandler() {
    ...
    return async (args) => {
        ...
        if (params.buildHierarchy) {
            ...
            const topLevel = Array.from(hierarchy.topLevel.clusters.values()).map((cluster) => {
                ...
                if (parentSubClusters) {
                    subclusters = Array.from(parentSubClusters.clusters.entries()).map(([subId, signals]) => {
                        totalSubclusters++;
                        return {
                            id: `${cluster.id}.${subId}`,
            ...
            const clusterSizes = Array.from(hierarchy.topLevel.clusters.values()).map((c) => c.signals.size);
        ...
        if (!validation.valid) {
            throw Errors.validationError('response', 'AnalyzeProjectResponse', validation.errors?.errors[0]?.message || 'Unknown validation error');
        }
        return response;
    };
}
