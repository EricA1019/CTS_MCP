...
function formatCTSPlan(report) {
    ...
    for (const fileReport of report.byFile) {
        ...
        const priorityBugs = fileReport.bugs.filter(b => b.severity === 'critical' || b.severity === 'high');
        ...
        const task = {
            ...
            acceptanceCriteria: priorityBugs.map(bug => `Line ${bug.line}: ${bug.name} - ${bug.message}`),
 ...
 */
function generateTechnicalNotes(bugs) {
