 ...
 */
export function createRenderArtifactHandler(engine) {
    return async (args) => {
        const params = RenderArtifactParamsSchema.parse(args);
        const result = await engine.renderArtifact(params.artifactType, params.data, params.metadata);
        const response = {
            success: true,
        ...
        if (!valid) {
            throw Errors.validationError('response', 'RenderArtifactResponse', errors?.errors[0]?.message || 'Invalid response structure');
        }
        return response;
    };
}
 ...
 */
export const renderArtifactTool = {
...
//# sourceMappingURL=render_artifact.js.map