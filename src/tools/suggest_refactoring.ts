...
import { validateToolResponse, SuggestRefactoringResponseSchema } from '../schemas.js';
...
export const suggestRefactoringTool: ToolDefinition = {
 ...
 * Create tool handler
 */
export function createSuggestRefactoringHandler(): ToolHandler {
  const monitor = new PerformanceMonitor();
  
  // Simple cache: projectPath → { graph, timestamp }
  const graphCache = new Map<string, { graph: any; timestamp: number }>();
  ...
  return async (args: Record<string, unknown>) => {
    ...
    const response = {
      ...
      toolName: 'CTS_Suggest_Refactoring' as const,
      ...
      result: {
        ...
        summary: {
          ...
          byType: {
            merge: allSuggestions.filter((s: any) => s.type === 'merge').length,
            rename: allSuggestions.filter((s: any) => s.type === 'rename').length,
            deprecate: allSuggestions.filter((s: any) => s.type === 'deprecate').length,
          },
          avgConfidence: topSuggestions.length > 0
            ? topSuggestions.reduce((sum: number, s: any) => sum + s.confidence, 0) / topSuggestions.length
            : 0,
        },
        performance: {
