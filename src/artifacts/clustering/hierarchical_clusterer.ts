...
import type { GraphNode, GraphLink, ClusterResult } from './community_detection.js';
import {
  type HierarchicalClusters,
  type LabeledCluster,
  type SubClusterResult,
...
export class HierarchicalClusterer {
  private labeler: TFIDFLabeler;
  private stats: ClusteringStats;
   ...
   */
  async clusterHierarchical(
    graph: SignalGraph,
    depth: number = 2,
    minSubClusterSize: number = 5
  ): Promise<HierarchicalClusters> {
    ...
    const labeledClusters = new Map<number, LabeledCluster>();
    ...
    const subClusters = new Map<number, SubClusterResult>();
