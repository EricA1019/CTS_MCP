...
export class TreeSitterBridge {
  ...
  async init(): Promise<void> {
    ...
    try {
      ...
      throw new Error(
        `Failed to initialize tree-sitter WASM: ${error instanceof Error ? error.message : String(error)}. ` +
   ...
   */
  private async loadGDScriptLanguage(): Promise<any> {
    try {
      ...
      try {
        ...
        const bindingPath = resolvePath(
          process.cwd(),
          'node_modules/tree-sitter-gdscript/build/Release/tree_sitter_gdscript_binding.node'
        );
  ...
  async parseFile(filePath: string): Promise<Parser.Tree> {
    if (!this.initialized || !this.parser) {
      throw new Error(
        'TreeSitterBridge not initialized. Call init() before parseFile().'
      );
    }
    ...
    try {
      ...
      if (lineCount >= 1000 && parseDuration > 250) {
        console.warn(
          `Performance target missed: ${lineCount} LOC parsed in ${parseDuration}ms (target: <250ms for 1000 LOC)`
        );
      }
  ...
  parseString(sourceCode: string): Parser.Tree {
    if (!this.initialized || !this.parser) {
      throw new Error(
        'TreeSitterBridge not initialized. Call init() before parseString().'
      );
    }
